...
//drag()sends scaleTarget event to scaleBy() of card, and lets the scaleBy() handle the scaling
    ...
    drag(event) {
		...
		//note: Vector3 is a class representing a 3D vector. A 3D vector is an ordered triplet of numbers (x, y, z)
		//in a world, the following colours represent the following axes
		//red = x
		//green = y
		//blue = z

        let origin = new THREE.Vector3(...event.ray.origin); //to define the origin (0, 0, 0) where the vector line will start
        let direction = new THREE.Vector3(...event.ray.direction); //to define the direction of the vector
        let ray = new THREE.Ray(origin, direction); //to define the ray that emits from an origin in a certain direction
        let intersectionPoint = ray.intersectPlane( //arguments are ( plane : Plane, target : Vector3 )
            this.intersectionPlane, //plane - the plane to intersect with
            new Microverse.THREE.Vector3() //target — the result will be copied into this Vector3.
        );
		...
		let nextScale = [...this.scaleAtDragStart]; //this is an array representing the current scale
        let s; //this variable gets gets a scalar value 
		
		//this checks which axis was chosen, and applies a scale multiplication corresponding to the tracked mouse movement
        if (this.actor._cardData.axis[0] === 1) {
            s = ratio[0];
			nextScale[0] = nextScale[0] * s;
        } else if (this.actor._cardData.axis[1] === 1) {
            s = ratio[1];
			nextScale[1] = nextScale[1] * s;
        } else if (this.actor._cardData.axis[2] === 1) {
            s = ratio[2];
			nextScale[2] = nextScale[2] * s;
        }
		
		//this sends the new scale to the actor
